...
fact.many<- function(manyglm.obj,nlv,res)
{
  ...
  return(list(BIC=BIC.out,logL=logL,theta=Th.out,sigma=Sig.out,loadings=Loadings,scores=Scores))
  
}

# nlv=2
# S.list=list(cor(spider$abund))
# N=100
factor_opt=function(nlv,S.list,full=FALSE,quick=FALSE,N){
  ...
  Sigma.gl=Theta.gl=list()
  Sigma.gl[[1]]=Sest ;Theta.gl[[1]]=Test
  A$sigma=Sest; A$theta=Test
  ...
  if(!(quick)){
    ...
    while((diff>eps & count<maxit)& any(!is.na(Theta.gl[[count]])))
    {
      ...
      Sigma.gl=Theta.gl=list()
      Sigma.gl[[count]]=Sest ;Theta.gl[[count]]=Test
      A$sigma=Sest; A$theta=Test
